...
import {AddSamplingComment} from '../../trazit-models/test-config-MicroEM-PendingSampling-AddSamplingComment';
...
const commonTests = () => {
    test('Trazit-MicroEM-PendingSampling-AddSamplingComment-Accept', async ({ page }, testInfo) => {
        await page.pause();
        let afterEachData = {
            textInNotif1:"",
            textInNotif2:"",
        ...
        await page.pause();
        await page.pause();
        await testInfo.attach(AddSamplingComment.screenShotsEmptyComment, {
            body: await page.screenshot({fullPage: true }), 
            contentType: ConfigSettingsAlternative.screenShotsContentType
        });
        await page.getByLabel(AddSamplingComment.fldComment.label).click();
        await page.pause();
        await page.pause();
        await page.getByLabel(AddSamplingComment.fldComment.label).fill(AddSamplingComment.fldComment.value);
        await page.pause();
        await page.pause();
        await testInfo.attach(AddSamplingComment.screenShotsFilledComment, {
            body: await page.screenshot({fullPage: true }), 
            contentType: ConfigSettingsAlternative.screenShotsContentType
        });
    ...
    test('Trazit-MicroEM-PendingSampling-AddSamplingComment-Cancel', async ({ page }, testInfo) => {
        await page.pause();
        await page.getByText(AddSamplingComment.selectSampleID, { exact: true }).click();
        await page.pause();
        await page.pause();
