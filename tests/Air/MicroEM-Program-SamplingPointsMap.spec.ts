...
import { SamplingPointsMaps } from '../../trazit-models/test-config-MicroEM-Program-SamplingPointsMap';
...
const commonTests = () => {
    test('Trazit-WaterMonitoring-ProgramList-SamplingPointsMap-Open', async ({ page }, testInfo) => {
        await page.pause();
        await page.pause();
        await page.getByLabel(SamplingPointsMaps.buttonName, {exact:true}).click();
        await page.pause();
        await page.pause();
        await testInfo.attach(SamplingPointsMaps.screenShotsSamplingPointsMap, {
