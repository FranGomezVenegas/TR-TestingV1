...
import { Deactivate } from '../../trazit-models/test-config-MicroEM-ActiveProductionLots-Deactivate';
...
const commonTests = () => {
    test('Trazit-MicroEM-ActiveProductionLots-Deactivate-Accept', async ({ page }, testInfo) => {
        await page.pause();
        let afterEachData = {
            textInNotif1:"",
            textInNotif2:"",
        ...
        await page.pause();
        await page.pause();
        await testInfo.attach(Deactivate.screenShotsSelectName, {
        ...
        await page.pause();
        await page.pause();
        await testInfo.attach(Deactivate.screenShotsPressButtonDeactivate, {
          body: await page.screenshot({fullPage: true }), 
          contentType: ConfigSettingsAlternative.screenShotsContentType
        });
    ...
    // Actualmente no se usa; no hay posibilidad de CANCELAR
    test('Trazit-MicroEM-ActiveProductionLots-Deactivate-Cancel', async ({ page }, testInfo) => {
      await page.pause();
      await page.getByText(Deactivate.selectName, { exact: true }).click();
      await page.pause();
      await page.pause();
