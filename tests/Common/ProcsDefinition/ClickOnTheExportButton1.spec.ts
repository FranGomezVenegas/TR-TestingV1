...
const commonTests = async (ConfigSettings, page, testInfo) => {
  ...
  // await handleActionNameInteraction(page, testInfo, Export);
  ...
  await test.step('Preparar para esperar la descarga', async () => {
      ...
      await test.step('Formatear la fecha y hora', async () => {
          ...
          const csvFileName = `${Export.screenShotTab
        ...
        await new Promise<void>((resolve, reject) => {
            ...
            exec(`cmd /c start "" "${Export.filePath}"`, async (err) => {
                ...
                try {
                    ...
                    const imgPath = path.join(Export.downloadPath, `${Export.screenShotTab}_${Export.csvFileName.replace('.csv', '')}.png`); // Usar el mismo nombre con la fecha
