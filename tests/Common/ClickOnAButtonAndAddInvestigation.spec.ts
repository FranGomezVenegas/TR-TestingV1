...
import { clickElementByText, attachScreenshot, clickDoButtonUserDialog, clickDoButtonJustification, clickJustificationButton, clickConfirmDialogButton, justificationPhrase, fillUserField, fillPasswordField, clickAcceptButton, clickDoButton, esignRequired } from '../1TRAZiT-Commons/actionsHelper';
...
const commonTests = async (ConfigSettings, page, testInfo) => {
  ...
  const expectedTexts = [
    addInvestigations.textInNotif1?.toLowerCase(),
    addInvestigations.textInNotif2?.toLowerCase(),
    addInvestigations.textInNotif3?.toLowerCase(),
  ];  
  ...
  // Proc Management
  await handleCardsInteraction(page, testInfo, addInvestigations);
  await handleSelectCard(page, testInfo, addInvestigations);
