...
import { decommision } from '../../trazit-models/test-config-instruments-decommision';
...
const commonTests = () => {
test('Decommision', async ({page}, testInfo) => {
    let afterEachData = {
      textInNotif1:"",
      textInNotif2:"",
      textInNotif3:""
    ...
    //let objectToGet=deactivate.tableFilterValue
    await page.getByText(decommision.selectInstruments, { exact: true }).click();
    await testInfo.attach(decommision.screenshotsSelectInstrument, {
      body: await page.screenshot({ fullPage: true }),
      contentType: ConfigSettingsAlternative.screenShotsContentType
    });
    await page.getByLabel(decommision.buttonDeactivate).click();
    await testInfo.attach(decommision.screenshotsDeactivate, {
      body: await page.screenshot({ fullPage: true }),
      contentType: ConfigSettingsAlternative.screenShotsContentType
    });
    await page.getByRole('textbox', { name: decommision.fldJustificationDeactivate.label }).fill(decommision.fldJustificationDeactivate.value);
    await page.getByRole('button', { name: decommision.fldButtonAccept.label }).click();
    ...
    afterEachData.textInNotif1=decommision.textInNotif1
    afterEachData.textInNotif2=decommision.textInNotif2
    afterEachData.textInNotif3=decommision.textInNotif3
  ...
  if (viewportWidth >= 1024) {
      ...
      await testInfo.attach(decommision.screenformNotifications, {
      body: await page.screenshot({ fullPage: true }),
      contentType: ConfigSettingsAlternative.screenShotsContentType
      });
      const notificationDiv = await page.locator(MenuInstrumentsControl.Notification.main.pageElementdiv).first();
      if (notificationDiv !== null) {
      await testInfo.attach(decommision.screenformLastNotifications, {
