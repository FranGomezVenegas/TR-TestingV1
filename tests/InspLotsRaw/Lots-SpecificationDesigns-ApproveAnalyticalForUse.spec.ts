...
import { ApproveAnalytical } from '../../trazit-models/test-config-Lots-SpecificationDesigns-ApproveAnalyticalForUse';
...
const commonTests = () => {
      test('Trazit-SpecificationDesigns-ApproveAnalyticalForUse-Accept', async ({ page }, testInfo) => {
        await page.pause();
        let afterEachData = {
            textInNotif1:"",
            textInNotif2:"",
        ...
        await page.getByRole('row', { name: ApproveAnalytical.buttonName.name }).getByLabel(ApproveAnalytical.buttonName.label).click();
        await page.pause();
        await page.pause();
        


        afterEachData.textInNotif1=ApproveAnalytical.textInNotif1
        afterEachData.textInNotif2=ApproveAnalytical.textInNotif2
        afterEachData.textInNotif3=ApproveAnalytical.textInNotif3
    ...
    if (viewportWidth >= 1024) {
        ...
        await testInfo.attach(ApproveAnalytical.screenformNotifications, {
            body: await page.screenshot({ fullPage: true }),
            contentType: ConfigSettingsAlternative.screenShotsContentType
        });
        const notificationDiv = await page.locator(MenuLots.Notification.main.pageElementdiv).first();
        if (notificationDiv !== null) {
            await testInfo.attach(ApproveAnalytical.screenformLastNotifications, {
