...
import { LotLock } from '../../trazit-models/test-config-Lots-View-Summary-InventoryRetain-LotLock';
...
const commonTests = () => {
    test('Trazit-LotsView-Summary-InventoryRetain-LotLock-Accept', async ({ page }, testInfo) => {
      await page.pause();
        await page.pause();
        ...
        await page.getByTitle(LotLock.buttonName.title, { exact: true }).getByLabel(LotLock.buttonName.label).click();
        await page.pause();
        await page.pause();
    })
}
