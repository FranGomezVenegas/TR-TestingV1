...
import { JustificationHandler } from '../1TRAZiT-Commons/1TRAZiT-ConfirmDialogs/JustificationPhrase-ProcsDefinition';

// Function with all tests.
const commonTests = () => {
    test('Trazit-ProceduresDefinition-RandD-Unlock', async ({ page }, testInfo) => {
        page.pause();
        page.pause();
        await unlockProcedure(page, procedures.RandD, testInfo);
        await page.pause();
        await page.pause();
        // Crear una instancia de JustificationHandler
        const justificationHandler = new JustificationHandler(page);
        // Manejar la justificación si está presente
        await justificationHandler.handleJustification('prueba', 'Accept');
        await page.pause();
        await page.close();
    });
  }
