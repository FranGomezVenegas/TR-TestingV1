...
import { new_inventory_lot } from '../../trazit-models/test-config-StockControl-ActiveInventroryLots-New';
...
const commonTests = () => {
  test('Trazit-StockControl-ActiveInventoryLots-Open', async ({ page }, testInfo) => {
      await page.pause();
      await page.pause();
...
await test.step('Verify network responses captured', async () => {
  ...
  test('Trazit-StockControl-ActiveInventoryLots-New-Accept', async ({ page }, testInfo) => {
    let afterEachData = {
      textInNotif1:"",
      textInNotif2:"",
      textInNotif3:"",
...
test.step('Attach Logger and NetworkInterceptor to the page', async () => {
  ...
  test('Trazit-StockControl-ActiveInventoryLots-New-Cancel', async ({ page }, testInfo) => {
    // Create instances of Logger and NetworkInterceptor
    const logger = new Logger();
    const networkInterceptor = new NetworkInterceptor();
    ...
    await testInfo.attach(new_inventory_lot.screenShotsFormFilled, {
      body: await page.screenshot({fullPage: true }), contentType: ConfigSettingsAlternative.screenShotsContentType});
    await page.getByRole('button', { name: new_inventory_lot.buttonCancel.label}).click();
    await page.pause();
    await page.pause();
    await testInfo.attach(new_inventory_lot.screenResult, {
      body: await page.screenshot({fullPage: true }), contentType: ConfigSettingsAlternative.screenShotsContentType});
