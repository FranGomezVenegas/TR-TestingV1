...
import { SetSampleDate } from '../../trazit-models/test-config-WaterMonitoring-PendingSampling-SetSampleDate';
import { callApiRunCompletion } from '../1TRAZiT-Commons/ApiCalls';
import { OpenProcedureWindow } from '../1TRAZiT-Commons/openProcedureWindow';

//Function with all tests.
const commonTests = () => {
test('Trazit-WaterMonitoring-PendingSampling-SetSampleDate', async ({ page }, testInfo) => {
  await page.pause();
  let afterEachData = {
    textInNotif1:"",
    textInNotif2:"",
  ...
  await page.getByText(SetSampleDate.selectIdSample, { exact: true }).click();
  await page.pause();
  await page.pause();
  await testInfo.attach(SetSampleDate.screenShotsSelectIdSample, {
    body: await page.screenshot({ fullPage: true }),
    contentType: ConfigSettingsAlternative.screenShotsContentType
  });
  await page.getByLabel(SetSampleDate.buttonSetSampleDate).click();
  await page.pause();
  await page.pause();
  await testInfo.attach(SetSampleDate.screenShotsButtonSetSampleDate, {
    body: await page.screenshot({ fullPage: true }),
    contentType: ConfigSettingsAlternative.screenShotsContentType
  });
  await page.getByLabel(SetSampleDate.buttonRefresh).click();
  await page.pause();
  await page.pause();
  await testInfo.attach(SetSampleDate.screenShotsButtonRefresh, {
    body: await page.screenshot({ fullPage: true }),
    contentType: ConfigSettingsAlternative.screenShotsContentType
  });

  afterEachData.textInNotif1=SetSampleDate.textInNotif1
  afterEachData.textInNotif2=SetSampleDate.textInNotif2
  afterEachData.textInNotif3=SetSampleDate.textInNotif3
  ...
  if (viewportWidth >= 1024) {
      ...
      await testInfo.attach(SetSampleDate.screenformNotifications, {
      body: await page.screenshot({ fullPage: true }),
      contentType: ConfigSettingsAlternative.screenShotsContentType
      });
      const notificationDiv = await page.locator(MenuInstrumentsControl.Notification.main.pageElementdiv).first();
      if (notificationDiv !== null) {
      await testInfo.attach(SetSampleDate.screenformLastNotifications, {
