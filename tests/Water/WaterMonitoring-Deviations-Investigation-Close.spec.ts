...
import { Close } from '../../trazit-models/test-config-WaterMonitoring-Deviations-Investigation-Close';
...
const commonTests = () => {
    test('Trazit-WaterMonitoring-Deviations-Investigation-Close', async ({ page }, testInfo) => {
        let afterEachData = {
            textInNotif1:"",
            textInNotif2:"",
            textInNotif3:"",
        ...
        await page.getByLabel(Close.buttonInvestigation).click();
        await page.pause();
        await page.pause();
        await testInfo.attach(Close.screenShotsInvestigations, {
            body: await page.screenshot({ fullPage: true }),
            contentType: ConfigSettingsAlternative.screenShotsContentType
        });
        await page.getByText(Close.selectCapaSystemID, { exact: true }).first().click();
        await page.pause();
        await page.pause();
        await testInfo.attach(Close.screenShotsSelect, {
            body: await page.screenshot({ fullPage: true }),
            contentType: ConfigSettingsAlternative.screenShotsContentType
        });
        await page.getByLabel(Close.buttonClose, { exact: true }).click();
        await page.pause();
        await page.pause();
        await testInfo.attach(Close.screenShotsClose, {
            body: await page.screenshot({ fullPage: true }),
            contentType: ConfigSettingsAlternative.screenShotsContentType
        });
        await page.pause();
