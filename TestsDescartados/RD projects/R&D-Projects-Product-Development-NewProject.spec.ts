...
import {NewProject} from '../../trazit-models/test-config-R&D-Projects-Product-Development-NewProject';
...
const commonTests = () => {
    test('Trazit-Projects-Development-NewProject-Accept', async ({ page }, testInfo) => {
        let afterEachData = {
            textInNotif1:"",
            textInNotif2:"",
            textInNotif3:"",
        ...
        await page.pause();
        await page.getByLabel(NewProject.Project).click();
        await page.pause();
        await page.pause();
        await page.getByLabel(NewProject.DailyEntries).click();
        await page.pause();
        await page.pause();
        await page.getByTitle(NewProject.buttonNewProject.label).getByLabel(NewProject.buttonNewProject.button).click();
        await page.pause();
        await page.pause();
        await testInfo.attach(NewProject.screenShotsformFilled, {
        ...
        await page.pause();
        await page.pause();
        await testInfo.attach(NewProject.screenShotsformEmpty, {
            body: await page.screenshot({ fullPage: true }),
            contentType: ConfigSettingsAlternative.screenShotsContentType
        });
        await page.getByRole('button', { name: NewProject.buttonAccept.label }).click();  
    ...
    test('Trazit-Projects-Development-NewProject-Cancel', async ({ page }, testInfo) => {
        await page.pause();
        await page.getByLabel(NewProject.Project).click();
        await page.pause();
        await page.pause();
        ...
        await page.getByRole('button', { name: NewProject.buttonCancel.label }).click();  
...
test.describe('Desktop Mode', () => {
  ...
  test.describe('Mobile mode Retrato', () => {
    test.beforeEach(async ({ page }, testInfo) => {
        ...
        const openWindow = new OpenProcedureWindow({ page });
        await page.waitForTimeout(3000);
        await openWindow.openWindowForMobile(page, testInfo, ConfigSettings, MenuRDProjects.RD.main, MenuRDProjects.RD.ProductDevelopment);
    });
    commonTests();
  });
  ...
  test.describe('Tablets mode Retrato', () => {
    test.beforeEach(async ({ page }, testInfo) => {
        ...
        const openWindow = new OpenProcedureWindow({ page });
        await page.waitForTimeout(3000);
        await openWindow.openWindowForTabletsModeRetrato(page, testInfo, ConfigSettings, MenuRDProjects.RD.mobile, MenuRDProjects.RD.ProductDevelopment);
    });
    commonTests();
  });
