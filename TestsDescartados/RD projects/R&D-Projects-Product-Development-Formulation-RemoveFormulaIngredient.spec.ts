...
import { RemoveIngredient } from '../../trazit-models/test-config-R&D-Projects-Product-Development-RemoveIngredient';
...
const commonTests = () => {
    test('Trazit-R&D-Projects-Development-RemoveIngredient', async ({ page }, testInfo) => {
        let afterEachData = {
            textInNotif1:"",
            textInNotif2:"",
            textInNotif3:"",
        ...
        await page.getByText('FormulaNameDev').click();        
        await page.getByText('FormulaNameDev').click();
        await page.getByRole('button', { name: 'DevName' }).click();
        await page.locator('#document').click();
        await page.getByRole('row', { name: '/images/activate.svg Almidón' }).getByLabel('playlist_remove').click();

        afterEachData.textInNotif1=RemoveIngredient.textInNotif1
        afterEachData.textInNotif2=RemoveIngredient.textInNotif2
        afterEachData.textInNotif3=RemoveIngredient.textInNotif3
        ...
        if (viewportWidth >= 1024) {
            ...
            await testInfo.attach(RemoveIngredient.screenformNotifications, {
            body: await page.screenshot({ fullPage: true }),
            contentType: ConfigSettingsAlternative.screenShotsContentType
            });
            const notificationDiv = await page.locator(MenuRDProjects.Notification.main.pageElementdiv).first();
            if (notificationDiv !== null) {
            await testInfo.attach(RemoveIngredient.screenformLastNotifications, {
