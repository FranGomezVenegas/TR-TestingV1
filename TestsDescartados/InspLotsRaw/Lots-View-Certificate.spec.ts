...
import { Certificate } from '../../trazit-models/test-config-lots-view-certificate';
...
const commonTests = () => {
  test('Trazit-Lots-LotsView-Search-Certificate', async ({ page }, testInfo) => {
    await page.pause();
    await page.pause();
    ...
    await page.pause();
    await page.getByLabel(LotsViewSearch.certificate).click();
    await page.pause();
    await page.pause();
    ...
    await testInfo.attach(LotsViewSearch.screenShotsSearch, {
        body: await page.screenshot({fullPage: true }), contentType: ConfigSettingsAlternative.screenShotsContentType});
    await page.pause();
    await page.getByLabel(Certificate.buttonCertificate).click();
    await page.pause();
    await page.pause();
    await testInfo.attach(Certificate.screenShotsButtonCertificate, {
        body: await page.screenshot({fullPage: true }), contentType: ConfigSettingsAlternative.screenShotsContentType});
    await page.pause();
    await page.pause();
    //await page.getByLabel(Certificate.buttonImprimir).click();
    await page.pause();
    await page.close();
    // await page.close();
  })
...
test.describe('Tablets mode Retrato', () => {
  test.beforeEach(async ({ page }, testInfo) => {
      ...
      const openWindow = new OpenProcedureWindow({ page });
      await page.waitForTimeout(3000);
      await openWindow.openWindowForTabletsModeRetrato(page, testInfo, ConfigSettings, MenuLots.Lots.mobile, MenuLots.Lots.lostView);
  });
  commonTests();
});
