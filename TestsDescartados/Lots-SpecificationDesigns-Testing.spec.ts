...
import { Testing } from '../../trazit-models/test-config-Lots-SpecificationDesigns-Testing';
...
const commonTests = () => {
  test('Trazit-SpecificationDesigns-Search-Testing', async ({ page }, testInfo) => {
    await page.pause();
    ...
    await page.pause();
    await page.pause();
    await page.getByLabel(Testing.buttonTesting).click();
    await page.pause();
    await page.pause();
    await page.pause();
    await testInfo.attach(Testing.screenShotsTesting, {
...
test.describe('Desktop Mode', () => {
  ...
  test.beforeEach(async ({ page }, testInfo) => {
      ...
      await testInfo.attach(platformMenuNames.procedure.main.screenShotsName, {
          body: await page.screenshot({fullPage: true }), contentType: ConfigSettings.screenShotsContentType});
      await page.locator('#dashboardprocedures').getByText('Procedures').hover();
