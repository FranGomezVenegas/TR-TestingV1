...
import { Config, newInvLotStdPrimariosSuccess,  newInvLotAlreadyExists, 
  newInvLotRegressionStdPrimario, tryTurnAvailableWhenNotQualified,
  newInvLotStdSecundatioNoQualifSuccess, turnAvailableWhenQualifNotRequired, newInvLotStdSecundariosAlternativeUnitsSuccess} from '../../trazit-models/test-config-stocks';
...
test.beforeEach(async ({ page }, testInfo) => {
  ...
  await page.pause();
  await page.getByText(newInvLotAlreadyExists.viewNameOrTextToGet).nth(0).click();   
  await page.pause(); 
});
...
test('newInvLotStdPrimariosSuccess', async ({ page }, testInfo) => {  
  let testModel=newInvLotStdPrimariosSuccess
